...
class AnnoRect(object):
    ...
    def sortCoords(self):　#x1,y1　：x2,y2の座標を整理する 必ずx1<x2,y1<y2 (x1,y1)は左上、（x2,y2）は右下を確保するための関数。
        if (self.x1>self.x2):
    ...
    def intersection(self, other):　#自分(正確値)と予測値重複したエリアを算出する。widthとhightをReturn
        self.sortCoords()
        other.sortCoords()
    ...
    def cover(self, other): #Iouではなく、予測値と正確値被った部(intersection)はっもと写真のどのぐらい%あるのを算出。
        nWidth = self.width()
        nHeight = self.height()
        iWidth, iHeight = self.intersection(other)              
        return float(iWidth * iHeight) / float(nWidth * nHeight)

    def overlap_pascal(self, other):　#自分（正確値）とOtherの比較(予測した値）　Iou(overlap)の算出。　
        self.sortCoords()　　#x1y1 x2y2のソート x1,y1は左上、x2y2は右したことを確保する。
        other.sortCoords()　#x1y1 x2y2のソート x1,y1は左上、x2y2は右したことを確保する。

        nWidth  = self.x2 - self.x1
        nHeight = self.y2 - self.y1
        iWidth, iHeight = self.intersection(other)　#重複エリアのWidthとHightを算出。
        interSection = iWidth * iHeight　#重複エリアの面積。
                
        union = self.width() * self.height() + other.width() * other.height() - interSection  #正確値と予測値の合計エリア。
                
        overlap = interSection * 1.0 / union　#overlapの算出。
        return overlap

    def isMatchingPascal(self, other, minOverlap): #Pascalでoverlap値算出、minOverlapと比較する。
        overlap = self.overlap_pascal(other)　#overlap値を算出する。ClassIDの意味はまだわからない。
        if (overlap >= minOverlap and (self.classID == -1 or other.classID == -1 or self.classID == other.classID)):
            return 1
        else:
            return 0

    def distance(self, other, aspectRatio=-1, fixWH='fixheight'):#正確値と予測値の距離 (とても小さいの点だと、被せいない可能性があるから...と思ってる、要確認)l4  
        if (aspectRatio!=-1):
            if (fixWH=='fixwidth'):
