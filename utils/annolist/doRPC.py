...
class AnnoGraph:
    ...
    def __init__(self, anno, det, ignore, style, minCover, minOverlap, maxDistance, ignoreOverlap):
        ...
        self.anno = anno　#GT 正確値
        self.det = det   #detection 予測値
        self.det.sortByScore("descending")

        # generate initial graph
        self.n = len(det.rects) #予測値の数
        self.m = len(anno.rects) #GT値の数
        ...
        self.match(style, minCover, minOverlap, maxDistance)　#Listの長さは予測値の数、予測値がTP(正しい)であればIndexを追加する。
        self.nextN = 0
    ...
    def match(self, style, minCover, minOverlap, maxDistance): #正しい予測値をself.M[]へ入れる
...
def asort(idlGT, idlDet, minWidth, minHeight, style, minCover, minOverlap, maxDistance, maxWidth=float('inf'), maxHeight=float('inf')):
    ...
    #ソートだけではないようですが　何のためにはわからない。
    for x,anno in enumerate(idlGT):　#GT→Ground Truth　正解値　毎画像のこと。

        imageFound = False
        filterIndex = -1
        for i,filterAnno in enumerate(idlDet):　　#予測画像に正確値が存在するかどうかのチック 重複しているような気がする。
        ...
        validGTRects = []
        for j in anno.rects:　#正解値毎四角形のこと。
            if (j.width() >= minWidth) and (j.height() >= minHeight) and (j.width() <= maxWidth) and (j.height() <= maxHeight):
                ...
                for m,frect in enumerate(idlDet[filterIndex].rects):　#一つ正確値（x1,y1,x2,y2に）対して、全部合格予測値をリストにいれる。
                    if(style == 0):　#std?
                        if (j.isMatchingStd(frect, minCover,minOverlap, maxDistance)):
                            overlap = j.overlap_pascal(frect)
                            matchingIndexes.append((m,overlap))

                    if(style == 1): #Iouで予測値が正しいかどうか算出。ソートで使うの？
                        if(j.isMatchingPascal(frect, minOverlap)): #isMatchingPascal AnnottionLibにある　minOverlapより高いであれば記録。
                            overlap = j.overlap_pascal(frect)      #overlap_pascal AnnottionLibにある
                            matchingIndexes.append((m, overlap))

                for m in xrange(len(matchingIndexes) - 1, -1, -1):#最大overlap一番被ってる予測値を残る。要調査。
                    ...
                    del idlDet[filterIndex].rects[matchingIndexes[m][0]]　#何を削除するはまだわからない。

        idlGT[x].rects = validGTRects

    #Sort out too small false positives 解析していない。min max設定していないから、０と無限大関係ないと思う。
...
def comp_prec_recall_all_params(annoIDL, detIDL, ignoreIDL, minWidth=0, minHeight=0, maxWidth=float('inf'), maxHeight=float('inf'),
    ...
    #正しい予測値をソートする。matchingStyle:Pascal Iou方式,ココでAnnoIDLが正しい予測値に変更されました？と思う。
    ...
    allRects = []
    missingFrames = 0
    for i in xrange(len(annoIDL)): #予測で正しいの正確値

        imageFound = False
        filterIndex = -1

        for j, detAnno in enumerate(detIDL):　#予測画像に正確値が存在するかどうかのチック
            if (suffixMatch(annoIDL[i].imageName, detIDL[j].imageName) and annoIDL[i].frameNr == detIDL[j].frameNr):　#suffixMatch:　AnnotationLibの関数　まだいみわからない。
                filterIndex = j
                imageFound = True
                break
        ...
        graphs.append(AnnoGraph(annoIDL[i], detIDL[filterIndex], ignoreIDL[i], matchingStyle, minCover, minOverlap, maxDistance, ignoreOverlap))　#わからない

        for j,rect in enumerate(detIDL[filterIndex]):
            newRect = detAnnoRect()
            newRect.imageName = anno.imageName
            newRect.frameNr = anno.frameNr
            newRect.rect = rect　#予測値のxy座標
    ...
    changed = False
    firstFP = True
    for i,nextrect in enumerate(allRects): #全て予測値で評価。
        ...
        changed = graphs[nextrect.imageIndex].addBB(nextrect) or changed #true or flaseの判断 何のためにはわからない。
        sf = sf + graphs[nextrect.imageIndex].maxflow()
        cd = cd + graphs[nextrect.imageIndex].consideredDets()
        iflow = iflow + graphs[nextrect.imageIndex].ignoredFlow()
        if(i in (1,3)):　#自分確認用
            print(changed,sf,cd,iflow)
...
def main():
    ...
    parser.add_option("-s", "--minScore",                   #評価しないのScore 試したが、評価結果に」影響がない。
    ...
    parser.add_option("--minOverlap", action="store", type="float", dest="minOverlap", default=0.5)　#evaluateのargs.iou_threshold　デフォルトで0.5
    ...
    parser.add_option("--ignoreOverlap", action="store", type="float", dest="ignoreOverlap", default = 0.9) #m無視する？よくわからない
    ...
    annoFile = args[0] #true eval files　「evalファイルのx1,y1,x2,y2のまま。」
    print "args_test"
    ...
    # Load files
    annoIDL = parse(annoFile)　#「evalファイルのx1,y1,x2,y2のまま。」
    detIDL = []               #予測値
    for dets in args[1:]:　#予測値の座標
        detIDL += parse(dets)
    ...
    if (not options.minScore == None): #予測値にMinScoreより小さい値を除く
